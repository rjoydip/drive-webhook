...
async function getKVNamespace(): Promise<KVNamespaceInfo | null> {
	const namespaces: KVNamespaceInfo[] =
		await $`wrangler kv namespace list`.json();
...
async function ensureKVNamespace(): Promise<string> {
	...
	const created: KVNamespaceInfo[] =
		await $`wrangler kv namespace create ${KV_NS}`.json();
...
async function main() {
	...
	interactive.success(
		`[8/${TOTAL_STEPS}] - Start Page Token (${startPageToken})`,
	);

	if (startPageToken) {
		await $`wrangler kv key put google_drive_start_page_token ${startPageToken} --namespace-id ${namespaceId} --remote`;
	}

	interactive.await(`[9/${TOTAL_STEPS}] - Upload secrets to Cloudflare`);

	const bulkResult =
		await $`wrangler secret bulk ${tempEnvFile} --name ${KV_NS}`.text();
